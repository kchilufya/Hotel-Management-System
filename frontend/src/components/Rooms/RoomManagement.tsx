...
const RoomManagement: React.FC = () => {
  ...
  const fetchRooms = useCallback(async () => {
    try {
      ...
      const roomsData = responseData.rooms || responseData || [];
      console.log('Setting rooms data:', roomsData);
      console.log('Room data sample (first room):', roomsData[0]);
      setRooms(Array.isArray(roomsData) ? roomsData : []);
      setTotalRooms(responseData.totalRooms || responseData.total || (Array.isArray(roomsData) ? roomsData.length : 0));
    } catch (error) {
  ...
  }, [token, page, rowsPerPage, searchTerm, statusFilter, typeFilter, floorFilter]);

  useEffect(() => {
    fetchRooms();
  }, [fetchRooms, page, rowsPerPage, searchTerm, statusFilter, typeFilter, floorFilter]);
  ...
  const handleSubmit = async () => {
    try {
      ...
      await fetchRooms(); // Make sure this completes before continuing
      console.log('Rooms fetch completed after save');
    } catch (error) {
      console.error('Error saving room:', error);
      
      let errorMessage = 'Error saving room';
      if (error instanceof Error) {
        errorMessage = `Error saving room: ${error.message}`;
        ...
        if ((error as any).response?.data?.message) {
          errorMessage = `Error saving room: ${(error as any).response.data.message}`;
        }
      }
